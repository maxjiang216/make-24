...
- **Train** (`train.html`): flash cards with spaced repetition over all 1820 sets.
  *Solvable?* asks whether a set can make 24 (1362 can, 458 can't); *Solve* covers the
  1362 solvable sets, ordered hardest first (fewest distinct solutions, then rarest
  strategy), easiest first, or by strategy. Any key stops the timer and shows the
  answer, then `→` / `←` grades it. Missed sets come back within a minute; fast right
  answers move a set out to 5 min, 30 min, 4 h, 1 day, 3 days, 8 days, 3 weeks, 2 months.
  `web/difficulty.js` (solutions per set) is generated by `node scripts/class-counts.js`.
